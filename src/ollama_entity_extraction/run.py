...
def save_sub_name_entities(ollama_model_name: str, name_count: int = 210):
    json_path = Path(ROOT_PATH, f"cejil_data/ner_json_files/person_names_{ollama_model_name}.json")
    sub_names = list(json.loads(json_path.read_text()).keys())[:name_count]
...
def run_unique_name_extraction_on_sub_data(ollama_model_name: str):
    ...
    extractor = OllamaNameExtractor()
    _, entities_dict = extractor.find_unique_names(entities_dict)
...
def run_unique_name_extraction_example():
    ...
    for file in tqdm(sorted(listdir(CACHED_DATA_PATH))):
        ...
        extractor.extract_entities(pdf_name, segment_boxes, entities_dict)
        break
    unique_names, entities_dict = extractor.find_unique_names(entities_dict)
...
def save_sub_organization_entities(ollama_model_name: str, name_count: int = 210):
    json_path = Path(ROOT_PATH, f"cejil_data/ner_json_files/organization_names_{ollama_model_name}.json")
    sub_names = list(json.loads(json_path.read_text()).keys())[:name_count]
    sub_json_data = {k: json.loads(json_path.read_text())[k] for k in sub_names}
...
def load_person_names():
    entities = json.loads(
        Path("/home/ali/projects/pdf-entity-extraction/cejil_data/ner_json_files/person_names_llama3.1.json").read_text()
    )
    entities_dict = EntitiesDict.from_dict(entities)
    print(list(entities_dict.keys())[:210])


if __name__ == "__main__":
    run_unique_organization_extraction_on_sub_data("llama3.1")
